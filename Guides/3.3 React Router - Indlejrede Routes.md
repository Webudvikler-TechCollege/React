...
export const AppRouter = () => {
  return (
    <Routes>
      ...
      <Route path="/products" element={<PosterPage />}>
        <Route path=":genre_id" element={<ProductList />}>
        <Route path=":genre_id/:product_id" element={<ProductDetails />} />
      </Route>
...
### Forklaring:
1. Den overordnede route viser elementet `<PosterPage>` på url'en `/products`.
2. Den første indlejrede route viser elementet `<PosterList>` på url'en `/products/category_id`. *Eksempel: /products/5*
3. Den anden indlejrede route viser elementet `<PosterDetails>` på url'en `/products/category_id/product_id`. *Eksempel: /products/5/10*
___
